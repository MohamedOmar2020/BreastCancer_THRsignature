...
Pheno_metabric_ERposHER2neg <- Pheno_metabric %>%
  dplyr::filter(ER.status.measured.by.IHC == 'Positve',
                HER2.Status == 'Negative') 

Expr_metabric_ERposHER2neg <- corrected_Expr_metabric[, rownames(Pheno_metabric_ERposHER2neg)]
all(colnames(Expr_metabric_ERposHER2neg) == rownames(Pheno_metabric_ERposHER2neg))

########################################
## Make a dataframe with survival time and event
########################################
os_event <- as.factor(Pheno_metabric_ERposHER2neg$Overall.Survival.Status)
os_time <- Pheno_metabric_ERposHER2neg$Overall.Survival..Months.
summary(os_time)

rfs_event <- as.factor(Pheno_metabric_ERposHER2neg$Relapse.Free.Status)
rfs_time <- Pheno_metabric_ERposHER2neg$Relapse.Free.Status..Months.
summary(rfs_time)


Data_metabric_ERposHER2neg <- as.data.frame(t(Expr_metabric_ERposHER2neg))
Data_metabric_ERposHER2neg$os_event <- os_event
Data_metabric_ERposHER2neg$os_time <- os_time
summary(Data_metabric_ERposHER2neg$os_time)

Data_metabric_ERposHER2neg$rfs_event <- rfs_event
Data_metabric_ERposHER2neg$rfs_time <- rfs_time
summary(Data_metabric_ERposHER2neg$rfs_time)

all(rownames(Data_metabric_ERposHER2neg) == rownames(Pheno_metabric_ERposHER2neg))


levels(Data_metabric_ERposHER2neg$os_event) <- c('low.risk', 'high.risk')
levels(Data_metabric_ERposHER2neg$rfs_event) <- c('low.risk', 'high.risk')
...
# Fit the GLM model
glm_model <- glm(os_event ~ LMNB2 + CDC20 + KIF2C + FAM64A + KIF4A + TPX2, 
                 data = Data_tcga_ERposHER2neg, family = binomial(link = "logit"))
...
tcga_prob <- glm_model %>% predict(Data_tcga_ERposHER2neg , type = "response")

### Threshold
thr <- coords(roc(Data_tcga_ERposHER2neg$os_event, tcga_prob, levels = c("low.risk", "high.risk"), direction = "<"), "best")["threshold"]
thr

### ROC Curve
roc_tcga <- roc(Data_tcga_ERposHER2neg$os_event, tcga_prob, plot = F, print.thres=thr$threshold, print.auc=TRUE, print.auc.col="black", ci = T, levels = c("low.risk", "high.risk"), direction = "<", col="blue", lwd=2, grid=TRUE)
roc_tcga

### Get predictions based on best threshold from ROC curve
tcga_classes <- ifelse(tcga_prob >= thr$threshold, "high.risk", "low.risk")
table(tcga_classes)
...
metabric_prob <- glm_model %>% predict(Data_metabric_ERposHER2neg, type = "response")

### ROC Curve
roc_metabric <- roc(Data_metabric_ERposHER2neg$os_event, metabric_prob, plot = F, print.thres=thr$threshold, print.auc=TRUE, print.auc.col="black", ci = T, levels = c("low.risk", "high.risk"), direction = "<", col="blue", lwd=2, grid=TRUE)
roc_metabric

### Get predictions based on best threshold from ROC curve
metabric_classes <- ifelse(metabric_prob >= thr$threshold, "high.risk", "low.risk")
table(metabric_classes)
metabric_classes <- factor(metabric_classes, levels = c('low.risk', 'high.risk'))

###
cm_metabric <- confusionMatrix(metabric_classes, Data_metabric_ERposHER2neg$os_event, positive = "high.risk", mode = "everything")
cm_metabric

## MCC
mcc_metabric <- mltools::mcc(pred = metabric_classes, actuals = Data_metabric_ERposHER2neg$os_event)
mcc_metabric
...
survival_tcga <- survival_tcga %>%
  mutate(quartiles = ntile(tcga_prob, 4), 
         quintiles = ntile(tcga_prob, 5),
         tertiles = ntile(tcga_prob, 3)
  )
...
## Keep only the relevant information (Metastasis Event and Time)
survival_metabric <- cbind(Pheno_metabric_ERposHER2neg[, c("Overall.Survival.Status", "Overall.Survival..Months.", "Relapse.Free.Status", "Relapse.Free.Status..Months.", 
...
survival_metabric <- survival_metabric %>%
  mutate(quartiles = ntile(metabric_prob, 4), 
         quintiles = ntile(metabric_prob, 5),
         tertiles = ntile(metabric_prob, 3)
  )
###########################################################################
...
tiff("./figures/E6_tcga_prognostic/E6_tcga_glmTrainedOnTCGA_os_two_predClasses.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/E6_tcga_prognostic/E6_tcga_glmTrainedOnTCGA_rfs_two_predClasses.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/E6_metabric_prognostic/E6_metabric_glmTrainedOnTCGA_os_two_predClasses.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/E6_metabric_prognostic/E6_metabric_glmTrainedOnTCGA_rfs_two_predClasses.tiff", width = 3000, height = 3000, res = 300)

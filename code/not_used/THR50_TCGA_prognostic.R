...
load('objs/THR50_model_logreg.rda')
...
Train_prob_THR50 <- THR50_model %>% predict(Data_tcga , type = "response")

### Threshold
thr_THR50 <- coords(roc(group_tcga, Train_prob_THR50, levels = c("0", "1"), direction = "<"), "best")["threshold"]
thr_THR50

### ROC Curve
ROCTrain_THR50 <- roc(group_tcga, Train_prob_THR50, plot = F, print.thres=thr_THR50$threshold, print.auc=TRUE, print.auc.col="black", ci = T, levels = c("0", "1"), direction = "<", col="blue", lwd=2, grid=TRUE)
ROCTrain_THR50
...
ConfusionTrain_THR50 <- confusionMatrix(Train_predClasses_THR50, group_tcga, positive = "1", mode = "everything")
ConfusionTrain_THR50

## MCC
MCC_Train_THR50 <- mltools::mcc(pred = Train_predClasses_THR50, actuals = group_tcga)
MCC_Train_THR50
...
CoxData_tcga <- CoxData_tcga %>%
  mutate(tcga_prob_THR50_quartiles = ntile(Train_prob_THR50, 4), 
         tcga_prob_THR50_quintiles = ntile(Train_prob_THR50, 5),
         tcga_prob_THR50_tertiles = ntile(Train_prob_THR50, 3)
  )
...
# OS
## tcga all genes
Fit_sig_tcga_os <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Train_predClasses_THR50, data = CoxData_tcga)


## by quartiles
Fit_sig_tcga_os_quartiles <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quartiles, data = CoxData_tcga)

## by quintiles
Fit_sig_tcga_os_quintiles <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quintiles, data = CoxData_tcga)

## by tertiles
Fit_sig_tcga_os_tertiles <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_tertiles, data = CoxData_tcga)

#############
# DFS
## tcga all genes
Fit_sig_tcga_DFS <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ Train_predClasses_THR50, data = CoxData_tcga)

## by quartiles
Fit_sig_tcga_DFS_quartiles <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quartiles, data = CoxData_tcga)

## by quintiles
Fit_sig_tcga_DFS_quintiles <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quintiles, data = CoxData_tcga)

## by tertiles
Fit_sig_tcga_DFS_tertiles <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_tertiles, data = CoxData_tcga)
...
CoxData_tcga_PAM_Q1vsQ4 <- CoxData_tcga_PAM %>%
  filter(tcga_prob_THR50_quartiles %in% c('1', '4'))

CoxData_tcga_PAM_Q1vsQ5 <- CoxData_tcga_PAM %>%
  filter(tcga_prob_THR50_quintiles %in% c('1', '5'))


# os: all
Fit_sig_tcga_os_PAM <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Train_predClasses_THR50 + Subtype, data = CoxData_tcga_PAM)

# os: quartiles: all
Fit_sig_tcga_os_quartiles_PAM <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quartiles + Subtype, data = CoxData_tcga_PAM)

# os: quintiles: all
Fit_sig_tcga_os_quintiles_PAM <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quintiles + Subtype, data = CoxData_tcga_PAM)

# os: quartiles: Q1 vs Q4
Fit_sig_tcga_os_Q1vsQ4_PAM <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quartiles + Subtype, data = CoxData_tcga_PAM_Q1vsQ4)

# os: quintiles: Q1 vs Q5
Fit_sig_tcga_os_Q1vsQ5_PAM <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quintiles + Subtype, data = CoxData_tcga_PAM_Q1vsQ5)

##########
# DFS: all
Fit_sig_tcga_dfs_PAM <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ Train_predClasses_THR50 + Subtype, data = CoxData_tcga_PAM)

# rfs: quartiles: all
Fit_sig_tcga_dfs_quartiles_PAM <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quartiles + Subtype, data = CoxData_tcga_PAM)

# rfs: quintiles: all
Fit_sig_tcga_dfs_quintiles_PAM <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quintiles + Subtype, data = CoxData_tcga_PAM)

# rfs: quartiles: Q1 vs Q4
Fit_sig_tcga_dfs_Q1vsQ4_PAM <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quartiles + Subtype, data = CoxData_tcga_PAM_Q1vsQ4)

# rfs: quintiles: Q1 vs Q5
Fit_sig_tcga_dfs_Q1vsQ5_PAM <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quintiles + Subtype, data = CoxData_tcga_PAM_Q1vsQ5)
...
# os: all
Fit_sig_tcga_os_ER <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Train_predClasses_THR50 + ER.Status.By.IHC, data = CoxData_tcga)

# os: quartiles: all
Fit_sig_tcga_os_quartiles_ER <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quartiles + ER.Status.By.IHC, data = CoxData_tcga)

# os: quintiles: all
Fit_sig_tcga_os_quintiles_ER <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quintiles + ER.Status.By.IHC, data = CoxData_tcga)

# os: quartiles: Q1 vs Q4
Fit_sig_tcga_os_Q1vsQ4_ER <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quartiles + ER.Status.By.IHC, data = CoxData_tcga_Q1vsQ4)

# os: quintiles: Q1 vs Q5
Fit_sig_tcga_os_Q1vsQ5_ER <- survfit(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_quintiles + ER.Status.By.IHC, data = CoxData_tcga_Q1vsQ5)

##############
# dfs: all
Fit_sig_tcga_dfs_ER <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ Train_predClasses_THR50 + ER.Status.By.IHC, data = CoxData_tcga)

# rfs: quartiles: all
Fit_sig_tcga_dfs_quartiles_ER <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quartiles + ER.Status.By.IHC, data = CoxData_tcga)

# rfs: quintiles: all
Fit_sig_tcga_dfs_quintiles_ER <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quintiles + ER.Status.By.IHC, data = CoxData_tcga)

# rfs: quartiles: Q1 vs Q4
Fit_sig_tcga_dfs_Q1vsQ4_ER <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quartiles + ER.Status.By.IHC, data = CoxData_tcga_Q1vsQ4)

# rfs: quintiles: Q1 vs Q5
Fit_sig_tcga_dfs_Q1vsQ5_ER <- survfit(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_quintiles + ER.Status.By.IHC, data = CoxData_tcga_Q1vsQ5)
...
tiff("./figures/THR50/TCGA/THR50_tcga_os_all.tiff", width = 3000, height = 3000, res = 300)
...
########
# by quartiles
tiff("./figures/THR50/TCGA/THR50_tcga_os_quartiles.tiff", width = 3000, height = 3000, res = 300)
...
#############
# by quintiles
tiff("./figures/THR50/TCGA/THR50_tcga_os_quintiles.tiff", width = 3000, height = 3000, res = 300)
...
#############
# by tertiles
tiff("./figures/THR50/TCGA/THR50_tcga_os_tertiles.tiff", width = 3000, height = 3000, res = 300)
...
# plot RFS
######################################
tiff("./figures/THR50/TCGA/THR50_tcga_DFS_allpairs.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/THR50/TCGA/THR50_tcga_DFS_quartiles.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/THR50/TCGA/THR50_tcga_DFS_quintiles.tiff", width = 3000, height = 3000, res = 300)
...
tiff("./figures/THR50/TCGA/THR50_tcga_DFS_tertiles.tiff", width = 3000, height = 3000, res = 300)
...
pdf("./figures/THR50_prognostic/tcga/THR50_tcga_os_PAM50.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_PAM,
           risk.table = FALSE,
           pval = TRUE,
           pval.size = 12,
           short.panel.labs = T,
           ...
           palette = 'jco',
           risk.table.y.text.col = FALSE,
           risk.table.y.text = FALSE, title = 'THR50 and tcga OS by PAM50 subtypes')
dev.off()
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_RFS_PAM50.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_dfs_THR50_PAM,
           risk.table = FALSE,
           pval = TRUE,
           pval.size = 12,
           short.panel.labs = T,
           ...
           palette = 'jco',
           risk.table.y.text.col = FALSE,
           risk.table.y.text = FALSE, title = 'THR50 and tcga DFS by PAM50 subtypes')
dev.off()
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_PAM50_quartiles.pdf", width = 12, height = 10, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_quartiles_PAM,
           risk.table = FALSE,
           pval = TRUE,
           pval.size = 12,
           short.panel.labs = T,
           ...
           legend.labs = c('Q1', 'Q2', 'Q3', 'Q4'),
           risk.table.y.text.col = FALSE,
           risk.table.y.text = FALSE, title = 'THR50 and tcga OS by PAM50 subtypes: quartiles')
dev.off()

####################################################
## OS: quintiles: all

tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_PAM50_quintiles.tiff", width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_os_THR50_quintiles_PAM,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "Subtype",
...
tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_PAM50_Q1vsQ4.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_os_THR50_Q1vsQ4_PAM,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "Subtype",
...
######################################################
# OS: quintiles: Q1 vs Q5
tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_PAM50_Q1vsQ5.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_os_THR50_Q1vsQ5_PAM,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "Subtype",
...
tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_dfs_PAM50_quartiles.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_dfs_THR50_quartiles_PAM,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "Subtype",
           ...
           ggtheme = theme_survminer(base_size = 18, font.x = c(18, 'bold.italic', 'black'), font.y = c(18, 'bold.italic', 'black'), font.tickslab = c(18, 'plain', 'black'), font.legend = c(18, 'bold', 'black')),
           palette = 'jco',
           risk.table.y.text = FALSE, title = 'THR50 and tcga DFS by PAM50 subtypes: quartiles')
dev.off()

####################################################
## RFS: quintiles: all

tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_dfs_PAM50_quintiles.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_dfs_THR50_quintiles_PAM,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "Subtype",
...
#############################
# RFS: quartiles: Q1 vs Q4
tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_dfs_PAM50_Q1vsQ4.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_dfs_THR50_Q1vsQ4_PAM,
           risk.table = FALSE,
           pval = TRUE,
           pval.size = 12,
           short.panel.labs = T,
...
#############################
# RFS: quintiles: Q1 vs Q5
tiff("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_dfs_PAM50_Q1vsQ5.tiff",  width = 3200, height = 2200, res = 300)
ggsurvplot(Fit_sig_tcga_dfs_THR50_Q1vsQ5_PAM,
           risk.table = FALSE,
           pval = T,
           short.panel.labs = T,
           facet.by = "Subtype",
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_ER.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_DFS_ER.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_dfs_THR50_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "ER.Status.By.IHC",
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_ER_quartiles.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_quartiles_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "ER.Status.By.IHC",
...
####################################################
# OS: quartiles: Q1 vs Q4
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_ER_Q1vsQ4.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_Q1vsQ4_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "ER.Status.By.IHC",
...
####################################################
# RFS: quartiles: all
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_ER_quartiles.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_quartiles_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "ER.Status.By.IHC",
...
####################################################
# RFS: quartiles: Q1 vs Q4
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_ER_Q1vsQ4.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_Q1vsQ4_ER,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "ER.Status.By.IHC",
...
# OS
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_X3.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
...
######################
# RFS
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_X3.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_X3_quartiles.pdf", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_quartiles_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
####################################################
## OS: quintiles: all
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_X3_quintiles.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_quintiles_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
####################################################
# OS: quartiles: Q1 vs Q4
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_X3_Q1vsQ4.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_Q1vsQ4_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
####################################################
## OS: quintiles: Q1 vs Q5
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_os_X3_Q1vsQ5.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_os_THR50_Q1vsQ5_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
###############################################################
# RFS: quartiles: all
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_X3_quartiles.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_quartiles_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
####################################################
## RFS: quintiles: all
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_X3_quintiles.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_quintiles_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_X3_Q1vsQ4.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_Q1vsQ4_X3,
           risk.table = FALSE,
           pval = F,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
####################################################
## RFS: quintiles: Q1 vs Q5
pdf("./figures/THR50_tcga/logreg/byClinicalGroup/THR50_tcga_rfs_X3_Q1vsQ5.tiff", width = 8, height = 8, onefile = F)
ggsurvplot(Fit_sig_tcga_rfs_THR50_Q1vsQ5_X3,
           risk.table = FALSE,
           pval = TRUE,
           short.panel.labs = T,
           facet.by = "X3.Gene.classifier.subtype",
...
Fit_sig_tcga_os_coxph_THR50_1 <- coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Train_prob_THR50_1, data = CoxData_tcga)
summary(Fit_sig_tcga_os_coxph_THR50_1)

#Fit_sig_tcga_os_coxph_THR50_2 <- coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ Train_prob_THR50_2, data = CoxData_tcga)
#summary(Fit_sig_tcga_os_coxph_THR50_2)
...
CoxData_tcga$tcga_prob_THR50_1_quartiles <- factor(CoxData_tcga$tcga_prob_THR50_1_quartiles, levels = c('1', '2', '3', '4'))
levels(CoxData_tcga$tcga_prob_THR50_1_quartiles) <- paste0('Q', levels(CoxData_tcga$tcga_prob_THR50_1_quartiles))

#CoxData_tcga$tcga_prob_THR50_2_quartiles <- factor(CoxData_tcga$tcga_prob_THR50_2_quartiles, levels = c('1', '2', '3', '4'))
#levels(CoxData_tcga$tcga_prob_THR50_2_quartiles) <- paste0('Q', levels(CoxData_tcga$tcga_prob_THR50_2_quartiles))
...
Fit_sig_tcga_os_coxph_THR50_1_quartiles <- coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_1_quartiles, data = CoxData_tcga)
summary_tcga_os_coxph_THR50_1_quartiles <- summary(Fit_sig_tcga_os_coxph_THR50_1_quartiles)

#Fit_sig_tcga_os_coxph_THR50_2_quartiles <- coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_2_quartiles, data = CoxData_tcga)
#summary_tcga_os_coxph_THR50_2_quartiles <- summary(Fit_sig_tcga_os_coxph_THR50_2_quartiles)

#summary_list_tcga_os_quartiles <- list(THR25 = summary_tcga_os_coxph_THR25_quartiles, THR50_1 = summary_tcga_os_coxph_THR50_1_quartiles, THR50_2 = summary_tcga_os_coxph_THR50_2_quartiles) 
...
# 
# HR_df_tcga_os_coxph_quartiles_THR50_1 <- as.data.frame(HR_list_tcga_os_coxph_quartiles$THR50_1)
# HR_df_tcga_os_coxph_quartiles_THR50_1$quartile <- gsub('.+quartiles', '', rownames(HR_df_tcga_os_coxph_quartiles_THR50_1))
# 
# HR_df_tcga_os_coxph_quartiles_THR50_2 <- as.data.frame(HR_list_tcga_os_coxph_quartiles$THR50_2)
# HR_df_tcga_os_coxph_quartiles_THR50_2$quartile <- gsub('.+quartiles', '', rownames(HR_df_tcga_os_coxph_quartiles_THR50_2))
# 
# # save the results
# write.csv(HR_df_tcga_os_coxph_quartiles_THR25, 'objs/HR/tcga/OS/THR25_quartiles_HR.csv')
# write.csv(HR_df_tcga_os_coxph_quartiles_THR50_1, 'objs/HR/tcga/OS/THR50_1_quartiles_HR.csv')
# write.csv(HR_df_tcga_os_coxph_quartiles_THR50_2, 'objs/HR/tcga/OS/THR50_2_quartiles_HR.csv')
...
Fit_sig_tcga_RFS_coxph_THR50_1 <- coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ Train_prob_THR50_1, data = CoxData_tcga)
summary(Fit_sig_tcga_RFS_coxph_THR50_1)

#Fit_sig_tcga_RFS_coxph_THR50_2 <- coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ Train_prob_THR50_2, data = CoxData_tcga)
#summary(Fit_sig_tcga_RFS_coxph_THR50_2)
...
Fit_sig_tcga_RFS_coxph_THR50_1_quartiles <- coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_1_quartiles, data = CoxData_tcga)
summary_tcga_RFS_coxph_THR50_1_quartiles <- summary(Fit_sig_tcga_RFS_coxph_THR50_1_quartiles)

#Fit_sig_tcga_RFS_coxph_THR50_2_quartiles <- coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_2_quartiles, data = CoxData_tcga)
#summary_tcga_RFS_coxph_THR50_2_quartiles <- summary(Fit_sig_tcga_RFS_coxph_THR50_2_quartiles)

#summary_list_tcga_RFS_quartiles <- list(THR25 = summary_tcga_RFS_coxph_THR25_quartiles, THR50_1 = summary_tcga_RFS_coxph_THR50_1_quartiles, THR50_2 = summary_tcga_RFS_coxph_THR50_2_quartiles) 
...
# 
# HR_df_tcga_RFS_coxph_quartiles_THR50_1 <- as.data.frame(HR_list_tcga_RFS_coxph_quartiles$THR50_1)
# HR_df_tcga_RFS_coxph_quartiles_THR50_1$quartile <- gsub('.+quartiles', '', rownames(HR_df_tcga_RFS_coxph_quartiles_THR50_1))
# 
# HR_df_tcga_RFS_coxph_quartiles_THR50_2 <- as.data.frame(HR_list_tcga_RFS_coxph_quartiles$THR50_2)
# HR_df_tcga_RFS_coxph_quartiles_THR50_2$quartile <- gsub('.+quartiles', '', rownames(HR_df_tcga_RFS_coxph_quartiles_THR50_2))
# 
# # save the results
# write.csv(HR_df_tcga_RFS_coxph_quartiles_THR25, 'objs/HR/tcga/RFS/THR25_quartiles_HR.csv')
# write.csv(HR_df_tcga_RFS_coxph_quartiles_THR50_1, 'objs/HR/tcga/RFS/THR50_1_quartiles_HR.csv')
# write.csv(HR_df_tcga_RFS_coxph_quartiles_THR50_2, 'objs/HR/tcga/RFS/THR50_2_quartiles_HR.csv')
...
CoxData_tcga_PAM$tcga_prob_THR50_1_quartiles <- factor(CoxData_tcga_PAM$tcga_prob_THR50_1_quartiles, levels = c('1', '2', '3', '4'))
levels(CoxData_tcga_PAM$tcga_prob_THR50_1_quartiles) <- paste0('Q', levels(CoxData_tcga_PAM$tcga_prob_THR50_1_quartiles))

#CoxData_tcga_PAM$tcga_prob_THR50_2_quartiles <- factor(CoxData_tcga_PAM$tcga_prob_THR50_2_quartiles, levels = c('1', '2', '3', '4'))
#levels(CoxData_tcga_PAM$tcga_prob_THR50_2_quartiles) <- paste0('Q', levels(CoxData_tcga_PAM$tcga_prob_THR50_2_quartiles))
...
CoxData_tcga_PAM_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles <- factor(CoxData_tcga_PAM_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles, levels = c('1', '4'))
levels(CoxData_tcga_PAM_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles) <- paste0('Q', levels(CoxData_tcga_PAM_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles))

#CoxData_tcga_PAM_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles <- factor(CoxData_tcga_PAM_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles, levels = c('1', '4'))
#levels(CoxData_tcga_PAM_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles) <- paste0('Q', levels(CoxData_tcga_PAM_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles))
...
CoxData_tcga_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles <- factor(CoxData_tcga_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles, levels = c('1', '4'))
levels(CoxData_tcga_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles) <- paste0('Q', levels(CoxData_tcga_Q1vsQ4_THR50_1$tcga_prob_THR50_1_quartiles))

#CoxData_tcga_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles <- factor(CoxData_tcga_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles, levels = c('1', '4'))
#levels(CoxData_tcga_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles) <- paste0('Q', levels(CoxData_tcga_Q1vsQ4_THR50_2$tcga_prob_THR50_2_quartiles))
...
lapply(split(CoxData_tcga_PAM, CoxData_tcga_PAM$Pam50...Claudin.low.subtype),
       function(x) summary(coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_1_quartiles, data = x)))

#lapply(split(CoxData_tcga_PAM, CoxData_tcga_PAM$Pam50...Claudin.low.subtype),
#       function(x) summary(coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_2_quartiles, data = x)))
...
#        function(x) summary(coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR25_quartiles, data = x)))
# 
# lapply(split(CoxData_tcga_PAM_Q1vsQ4_THR50_1, CoxData_tcga_PAM_Q1vsQ4_THR50_1$Pam50...Claudin.low.subtype),
#        function(x) summary(coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_1_quartiles, data = x)))
# 
# lapply(split(CoxData_tcga_PAM_Q1vsQ4_THR50_2, CoxData_tcga_PAM_Q1vsQ4_THR50_2$Pam50...Claudin.low.subtype),
#        function(x) summary(coxph(Surv(Overall.Survival..Months., Overall.Survival.Status) ~ tcga_prob_THR50_2_quartiles, data = x)))
...
lapply(split(CoxData_tcga_PAM, CoxData_tcga_PAM$Pam50...Claudin.low.subtype),
       function(x) summary(coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_1_quartiles, data = x)))

#lapply(split(CoxData_tcga_PAM, CoxData_tcga_PAM$Pam50...Claudin.low.subtype),
#       function(x) summary(coxph(Surv(Disease.Free..Months., Disease.Free.Status) ~ tcga_prob_THR50_2_quartiles, data = x)))

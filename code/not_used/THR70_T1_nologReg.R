...
# h = 9.71
Cluster5 <- cut(dend, h = 8.83)
T1_a <- Cluster5$lower[[1]] %>% labels()
T1_b <- Cluster5$lower[[2]] %>% labels()
...
levels(AnnAll_metabric$`THR clusters`) <- c('E3', 'E1', 'E2', 'E4', 'T1_a', 'T1_b')
...
ann_colors$`THR clusters` <- c("#66A61E", "#E7298A", "#1B9E77", "#D95F02", '#8a899a', '#6057cc')
names(ann_colors$`THR clusters`) <- levels(AnnAll_metabric$`THR clusters`)

# heatmap with clinical annotation
tiff('./figures/T1_noLogReg_THR70/THR70_heatmap_metabric_clusters_T1split_noLogReg.tiff', width=3000, height=2000, res = 300)
...
levels(Pheno_metabric$`THR clusters`) <- c('E3', 'E1', 'E2', 'E4', 'T1_a', 'T1_b')
table(Pheno_metabric$`THR clusters`)
...
############################################################################
# plot OS
cluster_colors <- c("#66A61E", "#E7298A", "#1B9E77", "#D95F02", '#8a899a', '#6057cc')

pdf("./figures/T1_noLogReg_THR70/metabric_os_5clusters_clustersFromHeatmap.pdf", width = 10, height = 8, onefile = F)
...
# plot RFS
pdf("./figures/T1_noLogReg_THR70/metabric_rfs_5clusters_clustersFromHeatmap.pdf", width = 10, height = 8, onefile = F)

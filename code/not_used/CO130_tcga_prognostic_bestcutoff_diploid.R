...
setdiff(CO130, rownames(Expr_tcga_refDip))

grep('^CTDNEP1', rownames(Expr_tcga_refDip), value = TRUE) # MINDY1
grep('^FAM46B', rownames(Expr_tcga_refDip), value = TRUE) # MINDY1
grep('^PIMREG', rownames(Expr_tcga_refDip), value = TRUE) # MINDY1

rownames(Expr_tcga_refDip)[rownames(Expr_tcga_refDip) == 'CTDNEP1'] <- 'HSA011916'
rownames(Expr_tcga_refDip)[rownames(Expr_tcga_refDip) == 'FAM46B'] <- 'TENT5B'
rownames(Expr_tcga_refDip)[rownames(Expr_tcga_refDip) == 'PIMREG'] <- 'FAM64A'
...
Expr_tcga_refDip <- Expr_tcga_refDip[, colnames(Expr_tcga_refDip) %in% rownames(Pheno_tcga)]
...
table(group_tcga)
Data_tcga <- as.data.frame(cbind(t(Expr_tcga_refDip), group_tcga))
...
tiff("./figures/CO130_tcga/CO130_tcga_os_bestCutoff_tertiles_prob_refDip.tiff", width = 3000, height = 3000, res = 300)
...
# 10 years
tiff("./figures/CO130_tcga/CO130_tcga_os_bestCutoff_tertiles_prob_refDip_10yrs.tiff", width = 3000, height = 3000, res = 300)
ggsurvplot(fit_os_bestcutoff_tertiles_prob, 
           data = CoxData_tcga, 
           risk.table = TRUE, pval = TRUE,
           xlim = c(0,120),

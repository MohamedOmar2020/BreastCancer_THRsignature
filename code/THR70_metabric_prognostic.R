...
data_to_plot_PAM50 <- CoxData_metabric_PAM %>%
  select(Train_prob_THR70, Pam50...Claudin.low.subtype) %>%
  dplyr::rename("PAM50 Groups" = Pam50...Claudin.low.subtype)

# Plotting the distribution of THR50 scores across PAM50 subtypes
tiff("./figures/THR70/metabric/THR70_Score_Distribution_by_PAM50_Subtype.tiff", width = 2000, height = 1600, res = 300)
ggplot(data_to_plot_PAM50, aes(x = Train_prob_THR70, fill = `PAM50 Groups`)) +
...
data_to_plot_X3 <- CoxData_metabric %>%
  select(Train_prob_THR70, X3.Gene.classifier.subtype) %>%
...
# Plotting the distribution of THR50 scores across PAM50 subtypes
tiff("./figures/THR70/metabric/THR70_Score_Distribution_by_X3_Subtype.tiff", width = 2000, height = 1600, res = 300)
ggplot(data_to_plot_X3, aes(x = Train_prob_THR70, fill = `X3 Gene Classifier Groups`)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.5, position = "identity") +
  geom_density(alpha = 0.7, adjust = 1, color = "black", size = 1) +

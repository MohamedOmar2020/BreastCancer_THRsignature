...
#############################################################################################################
# get cluster 5 with crossing curves
...
RFS_T1 <- as.factor(T1_pheno$Relapse.Free.Status)
Data_T1 <- as.data.frame(cbind(t(T1_expr), RFS_T1))
Data_T1$RFS_T1 <- as.factor(Data_T1$RFS_T1)
levels(Data_T1$RFS_T1) <- c('0', '1')

#####################################
# the model
THR70_model_T1 <- glm(as.formula((paste("RFS_T1 ~", paste(THR_70_fil, collapse = "+")))), data = Data_T1, family = "binomial")
summary(THR70_model_T1)

############################################################################
# Make predictions

Train_prob_THR70_T1 <- THR70_model_T1 %>% predict(Data_T1 , type = "response")

### Threshold
thr_THR70_T1 <- coords(roc(RFS_T1, Train_prob_THR70_T1, levels = c("0", "1"), direction = "<"), "best")["threshold"]
thr_THR70_T1

### ROC Curve
ROCTrain_THR70_T1 <- roc(RFS_T1, Train_prob_THR70_T1, plot = F, print.thres=thr_THR70_T1$threshold, print.auc=TRUE, print.auc.col="black", ci = T, levels = c("0", "1"), direction = "<", col="blue", lwd=2, grid=TRUE)
ROCTrain_THR70_T1

### Get predictions based on best threshold from ROC curve
predClasses_THR70_T1 <- ifelse(Train_prob_THR70_T1 >= thr_THR70_T1$threshold, "1", "0")
table(predClasses_THR70_T1)
predClasses_THR70_T1 <- factor(predClasses_THR70_T1, levels = c('0', '1'))
...
# plot OS
tiff("./figures/T1_logReg_RFS_THR70/THR70_metabric_os_T1.tiff", width = 3000, height = 3000, res = 300)
ggsurvplot(Fit_sig_metabric_os_THR70_T1,
           ...
           title = 'OS in METABRIC c5'
)
dev.off()

######################################
# plot RFS
tiff("./figures/T1_logReg_RFS_THR70/THR70_metabric_RFS_T1.tiff", width = 3000, height = 3000, res = 300)
ggsurvplot(Fit_sig_metabric_RFS_THR70_T1,
           ...
           risk.table.y.text = FALSE, 
           palette = 'jco',
           title = 'RFS in METABRIC c5'
)
dev.off()

##########################################################################################
##########################################################################################
## recombine c5 with the rest 
T1_pheno$`THR clusters` <- as.factor(T1_pheno$predClasses_THR70_T1)
levels(T1_pheno$`THR clusters`) <- c('T1_a', 'T1_b')
...
#levels(survival_metabric$THR.clusters) <- c('T1_a', 'T1_b', 'E3', 'E1', 'E2', 'E4')
table(survival_metabric$THR.clusters)

#####################
# match cluster colors in heatmap and KM plots
# look at the original cluster colors then modify them for survival
ann_colors$`THR clusters` # the names of the classes are wrongly assigned
levels(survival_metabric$THR.clusters)
#cluster_colors <- as.vector(ann_colors$`THR clusters`)

# T1 color is #1B9E77 make two colors of this for T1_a and T1_b
T1_colors <- colorRampPalette(c("#1B9E77", "red")) 

cluster_colors <- c('#8a899a', '#6057cc', '#66a61e', "#E7298A", "#1B9E77", "#D95F02")
...
pdf("./figures/T1_logReg_RFS_THR70/metabric_os_5clusters_newT1.pdf", width = 10, height = 8, onefile = F)
...
## RFS: 
pdf("./figures/T1_logReg_RFS_THR70/metabric_rfs_5clusters_newT1.pdf", width = 10, height = 8, onefile = F)
...
# Create annotation for columns/samples based on some clinical variables:
Pheno_metabric5 <- merge(x = c5, y = Pheno_metabric, by="Sample.ID", all.y = TRUE)

Pheno_metabric5 <- Pheno_metabric5 %>% 
  mutate(`THR clusters` = as.factor(`THR clusters`), THR.clusters = as.factor(THR.clusters)) %>%
  mutate(THR.clusters = coalesce(THR.clusters,`THR clusters`))

rownames(Pheno_metabric5) <- Pheno_metabric5$Sample.ID

Pheno_metabric_forHeatmap <- Pheno_metabric5
rownames(Pheno_metabric_forHeatmap) <- NULL
...
# add the cluster info to the Ann dataframe and re-plot the heatmap
all(rownames(Pheno_metabric5) == rownames(AnnAll_metabric))
levels(Pheno_metabric5$THR.clusters) <- c('T1_a', 'T1_b', 'E3', 'E1', 'E4', 'E2')
AnnAll_metabric$THR.clusters <- Pheno_metabric5$THR.clusters
table(AnnAll_metabric$THR.clusters)
...
tiff('./figures/c5/THR70_heatmap_metabric_clusters_newc5.tiff', width=3000, height=2000, res = 300)
